    ...
    if (parties.length > 0) {
      lines.push(
        `Political parties tracked for ${country.name}: ${parties
          .map((party) => `${party.name} (${party.descriptor})`)
          .join(' | ')}.`
      );

      parties.forEach((party) => {
        const relevantStances = topic
          ? party.stances.filter((stance) => stance.topicId === topic.id)
          : party.stances;

        relevantStances.forEach((stance) => {
          const stanceTopic = getTopicById(stance.topicId);

          lines.push(
            `- ${party.name} on ${stanceTopic?.label ?? stance.topicId}: ${stance.stance} — ${stance.note}`
          );
        });
      });
    }
